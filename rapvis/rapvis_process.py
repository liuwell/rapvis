...
def process(R1, R2, output, adapter, threads, libpath, minlen, trim5):
	...
	### trimmomatic
	realpath = sys.path[0]
	subprocess.call("trimmomatic PE -threads %d -phred33 %s %s %s %s %s %s ILLUMINACLIP:../library/adapter/%s:1:30:10:5 SLIDINGWINDOW:4:20 MINLEN:%d HEADCROP:%d 2> %s" % (threads, R1, R2, out_R1_p, out_R1_u, out_R2_p, out_R2_u, adapter, minlen, trim5, out_log), shell=True)
...
if __name__ == '__main__':
	...
	process(args.f1, args.f2, args.output, args.adapter, args.threads, args.libraryPath, args.minlen, args.trim5)
